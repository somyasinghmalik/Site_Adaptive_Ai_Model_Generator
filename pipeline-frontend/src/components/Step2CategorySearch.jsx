...
export default function Step2CategorySearch({ targetUrl, onSearchComplete, onBack }) {
  ...
  const fetchAllCategories = useCallback(async () => {
    try {
      const res = await fetch(`http://127.0.0.1:8000/api/categories?user_id=${encodeURIComponent(userId)}&url=${encodeURIComponent(targetUrl)}`);
      if (res.ok) {
    ...
    const checkInitialEngineStatus = async () => {
      try {
        ...
        const res = await fetch(
          `http://127.0.0.1:8000/api/status?job_id=${jobId}`
        );
        if (res.ok) {
          const data = await res.json();
  ...
  const generateImage = async (prompt) => {
    ...
    formData.append("generation_options", JSON.stringify(options));
  ...
  const buildDefaultOptions = () => {
    ...
    Object.keys(OPTION_LIBRARY).forEach(category => {
      defaults[category] = {
        mode: "AI_SELECTED",
        custom: ""
      };
