...
from migrate_json_to_db import sync_json_to_db
load_dotenv()
...
# -----------------------------
def replicate_consumer_worker(job_id, output_json_path, data_queue):
    print(
    ...
    while True:
        ...
        with data_lock:
            for data_entry in site_data_store[job_id]["pages"]:
            ...
            with open(output_json_path, "w", encoding="utf-8") as f:
                json.dump(site_data_store[job_id], f, indent=2)
...
def capture_screenshots(page, url, output_base, item_data, screenshot_dir, data_queue):
    try:
...
def run_scraper_pipeline(
    ...
    site_data_store[job_id] = {
        "global_brand_visual_blueprint": "Pending initialization...",
        "global_layout_aspect_ratio": "16:9",
        "pages": []
    }
    ...
    if os.path.exists(output_json_path):
        ...
        if not force_rescrape:
            ...
            with open(output_json_path, "r", encoding="utf-8") as f:
                site_data_store[job_id] = json.load(f)

            print(
                f"SUCCESS: Loaded local data map. Current Saved Blueprint Summary:\n"
                f"> {site_data_store[job_id].get('global_brand_visual_blueprint')}\n"
            )
    ...
    with data_lock:
        site_data_store[job_id]["global_brand_visual_blueprint"] = "Pending initialization..."
        site_data_store[job_id]["pages"] = []
    ...
    with sync_playwright() as p:
        ...
        if brand_ratio_match:
            site_data_store[job_id]["global_layout_aspect_ratio"] = brand_ratio_match.group(1)
            site_data_store[job_id]["global_brand_visual_blueprint"] = BRAND_VISUAL_PROMPT.replace(brand_ratio_match.group(0), "").strip()
        else:
            site_data_store[job_id]["global_layout_aspect_ratio"] = "16:9"
            site_data_store[job_id]["global_brand_visual_blueprint"] = BRAND_VISUAL_PROMPT

        print(f"\n[Brand Blueprint Saved To Master Key]:\n> {site_data_store[job_id]['global_brand_visual_blueprint']}\n")
    ...
    for t in consumer_threads:
        t.join()
    print(f"\nSUCCESS: Pipeline finished execution perfectly. Dataset saved: {output_json_path}")
    sync_json_to_db(output_json_path)



def playwright_worker(screenshot_dir, job_id, crawl_queue, data_queue):
    with sync_playwright() as p:
        ...
        while True:
            ...
            with data_lock:
                site_data_store[job_id]["pages"].append(data_entry)
